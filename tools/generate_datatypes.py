...
static UA_INLINE UA_StatusCode %s_copy(const %s *src, %s *dst) { ''' + \
    ...
    def encoding_h(self, typeTableName):
        return '''static UA_INLINE UA_StatusCode %s_encodeBinary(const %s *src, UA_ByteString *dst, size_t *offset) { size_t skip=0; return UA_encodeBinary(src, %s, dst, offset, &skip); }
