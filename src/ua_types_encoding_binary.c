...
static UA_StatusCode UA_Array_decodeBinaryMembuf(const UA_ByteString *src, size_t *UA_RESTRICT offset,
    ...
    *dst = UA_balloc(dataType->memSize * (size_t)noElements);
	uintptr_t ptr = (uintptr_t) *dst;
    UA_StatusCode retval = UA_STATUSCODE_GOOD;
	for (UA_Int32 i = 0; i < noElements && retval == UA_STATUSCODE_GOOD; i++) {
		retval = UA_decodeBinaryMembuf(src, offset, (void*) ptr, dataType);
		ptr += dataType->memSize;
	}
	return retval;
}
...
UA_StatusCode UA_Float_decodeBinary(UA_ByteString const *src, size_t *offset,
	...
	if (biasedExponent >= 127)
		...
		*dst = (UA_Float) sign * 2.0 * (1.0 + mantissa / 128.0)
				/ ((UA_Float) (biasedExponent - 127));
	*offset += 4;
	return UA_STATUSCODE_GOOD;
}
...
static UA_StatusCode UA_String_decodeBinaryMembuf(UA_ByteString const *src, size_t *UA_RESTRICT offset, UA_String *dst) {
	UA_Int32 length = -1;
    UA_StatusCode retval = UA_Int32_decodeBinary(src, offset, &length);
	if (length <= 0 || retval != UA_STATUSCODE_GOOD) {
        dst->data = UA_NULL;
		if (length == 0)
			dst->length = 0;
		else
...
#define UA_ByteString_decodeBinaryMembuf UA_String_decodeBinaryMembuf
...
static UA_StatusCode UA_NodeId_decodeBinaryMembuf(UA_ByteString const *src, size_t *UA_RESTRICT offset, UA_NodeId *dst) {
	// temporary variables to overcome decoder's non-endian-saveness for datatypes with different length
	UA_Byte dstByte = 0;
	UA_UInt16 dstUInt16 = 0;
	UA_Byte encodingByte = 0;
	...
	case UA_NODEIDTYPE_STRING: // Table 6, second entry
		dst->identifierType = UA_NODEIDTYPE_STRING;
		retval |= UA_UInt16_decodeBinary(src, offset, &dst->namespaceIndex);
		retval |= UA_String_decodeBinaryMembuf(src, offset, &dst->identifier.string);
		break;
	case UA_NODEIDTYPE_GUID: // Table 6, third entry
		dst->identifierType = UA_NODEIDTYPE_GUID;
		retval |= UA_UInt16_decodeBinary(src, offset, &dst->namespaceIndex);
	...
	case UA_NODEIDTYPE_BYTESTRING: // Table 6, "OPAQUE"
		dst->identifierType = UA_NODEIDTYPE_BYTESTRING;
		retval |= UA_UInt16_decodeBinary(src, offset, &dst->namespaceIndex);
		retval |= UA_ByteString_decodeBinaryMembuf(src, offset, &dst->identifier.byteString);
		break;
	default:
		UA_NodeId_init(dst);
		retval |= UA_STATUSCODE_BADINTERNALERROR; // the client sends an encodingByte we do not recognize
...
UA_StatusCode UA_ExpandedNodeId_decodeBinary(UA_ByteString const *src, size_t *UA_RESTRICT offset, UA_ExpandedNodeId *dst) {
...
static UA_StatusCode UA_ExpandedNodeId_decodeBinaryMembuf(UA_ByteString const *src, size_t *UA_RESTRICT offset, UA_ExpandedNodeId *dst) {
	UA_ExpandedNodeId_init(dst);
	// get encodingflags and leave a "clean" nodeidtype
	if ((UA_Int32) *offset >= src->length)
		return UA_STATUSCODE_BADDECODINGERROR;
	...
	UA_StatusCode retval = UA_NodeId_decodeBinaryMembuf(src, offset, &dst->nodeId);
	if (encodingByte & UA_EXPANDEDNODEID_NAMESPACEURI_FLAG) {
		dst->nodeId.namespaceIndex = 0;
		retval |= UA_String_decodeBinaryMembuf(src, offset, &dst->namespaceUri);
	}
	if (encodingByte & UA_EXPANDEDNODEID_SERVERINDEX_FLAG)
		retval |= UA_UInt32_decodeBinary(src, offset, &dst->serverIndex);
	return retval;
...
/* QualifiedName */
UA_StatusCode UA_QualifiedName_encodeBinary(UA_QualifiedName const *src, UA_ByteString* dst, size_t *UA_RESTRICT offset) {
...
static UA_StatusCode UA_QualifiedName_decodeBinaryMembuf(UA_ByteString const *src, size_t *UA_RESTRICT offset, UA_QualifiedName *dst) {
	UA_StatusCode retval = UA_UInt16_decodeBinary(src, offset, &dst->namespaceIndex);
	retval |= UA_String_decodeBinaryMembuf(src, offset, &dst->name);
	return retval;
}
...
UA_StatusCode UA_LocalizedText_encodeBinary(UA_LocalizedText const *src, UA_ByteString *dst, size_t *UA_RESTRICT offset) {
...
UA_StatusCode UA_LocalizedText_decodeBinary(UA_ByteString const *src, size_t *UA_RESTRICT offset, UA_LocalizedText *dst) {
...
static UA_StatusCode UA_LocalizedText_decodeBinaryMembuf(UA_ByteString const *src, size_t *UA_RESTRICT offset, UA_LocalizedText *dst) {
	UA_LocalizedText_init(dst);
	UA_Byte encodingMask = 0;
	UA_StatusCode retval = UA_Byte_decodeBinary(src, offset, &encodingMask);
	if (encodingMask & UA_LOCALIZEDTEXT_ENCODINGMASKTYPE_LOCALE)
		retval |= UA_String_decodeBinaryMembuf(src, offset, &dst->locale);
	if (encodingMask & UA_LOCALIZEDTEXT_ENCODINGMASKTYPE_TEXT)
		retval |= UA_String_decodeBinaryMembuf(src, offset, &dst->text);
	return retval;
}

/* ExtensionObject */
UA_StatusCode UA_ExtensionObject_encodeBinary(UA_ExtensionObject const *src, UA_ByteString *dst, size_t *UA_RESTRICT offset) {
...
UA_StatusCode UA_ExtensionObject_decodeBinary(UA_ByteString const *src, size_t *UA_RESTRICT offset, UA_ExtensionObject *dst) {
...
static UA_StatusCode UA_ExtensionObject_decodeBinaryMembuf(UA_ByteString const *src, size_t *UA_RESTRICT offset, UA_ExtensionObject *dst) {
	UA_ExtensionObject_init(dst);
	UA_Byte encoding = 0;
	UA_StatusCode retval = UA_NodeId_decodeBinaryMembuf(src, offset, &dst->typeId);
	retval |= UA_Byte_decodeBinary(src, offset, &encoding);
	dst->encoding = encoding;
	switch (dst->encoding) {
	case UA_EXTENSIONOBJECT_ENCODINGMASK_NOBODYISENCODED:
	...
	case UA_EXTENSIONOBJECT_ENCODINGMASK_BODYISBYTESTRING:
	case UA_EXTENSIONOBJECT_ENCODINGMASK_BODYISXML:
		retval |= UA_ByteString_decodeBinaryMembuf(src, offset, &dst->body);
		break;
	default:
		return UA_STATUSCODE_BADDECODINGERROR;
	}
...
static UA_StatusCode UA_Variant_decodeBinaryMembuf(UA_ByteString const *src, size_t *UA_RESTRICT offset, UA_Variant *dst);

static UA_StatusCode UA_DataValue_decodeBinaryMembuf(UA_ByteString const *src, size_t *UA_RESTRICT offset, UA_DataValue *dst) {
	UA_DataValue_init(dst);
	UA_StatusCode retval = UA_Byte_decodeBinary(src, offset, (UA_Byte*) dst);
	if (dst->hasValue)
		retval |= UA_Variant_decodeBinaryMembuf(src, offset, &dst->value);
	if (dst->hasStatus)
		retval |= UA_StatusCode_decodeBinary(src, offset, &dst->status);
	if (dst->hasSourceTimestamp)
		retval |= UA_DateTime_decodeBinary(src, offset, &dst->sourceTimestamp);
...
static UA_StatusCode UA_Variant_decodeBinaryMembuf(UA_ByteString const *src, size_t *UA_RESTRICT offset, UA_Variant *dst) {
	...
	if (isArray) {
		...
		UA_Int32 arraySize = -1;
		retval |= UA_Int32_decodeBinary(src, offset, &arraySize);
		retval |= UA_Array_decodeBinaryMembuf(src, offset, arraySize, &dst->data, dataType);
		if (retval != UA_STATUSCODE_GOOD)
			return retval;
		dst->arrayLength = arraySize;
		dst->type = dataType;
		...
		const UA_DataType *dataType = &UA_TYPES[typeIndex];
		retval |= UA_Array_decodeBinaryMembuf(src, offset, 1, &dst->data, dataType);
		if (retval != UA_STATUSCODE_GOOD)
			return retval;
		dst->type = dataType;
	} else {
		...
		const UA_DataType *dataType = UA_NULL;
		if (typeId.namespaceIndex == 0 &&
            EOencodingByte == UA_EXTENSIONOBJECT_ENCODINGMASK_BODYISBYTESTRING) {
			/* find the datatype (ins ns0) of the extension object */
			for (typeIndex = 0; typeIndex < UA_TYPES_COUNT; typeIndex++) {
				if (UA_NodeId_equal(&typeId, &UA_TYPES[typeIndex].typeId)) {
					dataType = &UA_TYPES[typeIndex];
		...
		retval = UA_decodeBinaryMembuf(src, offset, dst->data, dataType);
		if (retval != UA_STATUSCODE_GOOD)
			return retval;
		dst->type = dataType;
		dst->arrayLength = -1;
	...
	if (isArray && (encodingByte & UA_VARIANT_ENCODINGMASKTYPE_DIMENSIONS)) {
		retval = UA_Int32_decodeBinary(src, offset, &dst->arrayDimensionsSize);
		retval |= UA_Array_decodeBinaryMembuf(src, offset, dst->arrayDimensionsSize, &dst->data, &UA_TYPES[UA_TYPES_INT32]);
		if (retval != UA_STATUSCODE_GOOD)
			dst->arrayDimensionsSize = -1;
	}
	return retval;
...
/* DiagnosticInfo */
UA_StatusCode UA_DiagnosticInfo_encodeBinary(const UA_DiagnosticInfo *src, UA_ByteString * dst, size_t *UA_RESTRICT offset) {
	UA_StatusCode retval = UA_Byte_encodeBinary((const UA_Byte *) src, dst, offset);
...
UA_StatusCode UA_DiagnosticInfo_decodeBinary(UA_ByteString const *src, size_t *UA_RESTRICT offset, UA_DiagnosticInfo *dst) {
...
static UA_StatusCode UA_DiagnosticInfo_decodeBinaryMembuf(UA_ByteString const *src, size_t *UA_RESTRICT offset, UA_DiagnosticInfo *dst) {
	UA_DiagnosticInfo_init(dst);
	UA_StatusCode retval = UA_Byte_decodeBinary(src, offset, (UA_Byte*) dst);
	if (!retval)
		return retval;
	...
	if (dst->hasInnerDiagnosticInfo) {
        ...
        if(!dst->innerDiagnosticInfo)
            return retval | UA_STATUSCODE_BADOUTOFMEMORY;
...
UA_StatusCode UA_encodeBinary(const void *src, const UA_DataType *dataType, UA_ByteString *dst, size_t *UA_RESTRICT offset) {
	uintptr_t ptr = (uintptr_t) src;
	UA_StatusCode retval = UA_STATUSCODE_GOOD;
	...
	for (size_t i = 0; i < membersSize && retval == UA_STATUSCODE_GOOD; i++) {
		...
		if (member->isArray) {
			...
			retval = UA_Array_encodeBinary(*(void * const *) ptr, noElements,
					memberType, dst, offset);
			ptr += sizeof(void*);
			continue;
		}
...
UA_StatusCode UA_decodeBinary(const UA_ByteString *src, size_t *UA_RESTRICT offset, void *dst, const UA_DataType *dataType) {
	uintptr_t ptr = (uintptr_t) dst;
	UA_StatusCode retval = UA_STATUSCODE_GOOD;
	UA_Byte membersSize = dataType->membersSize;
	size_t i = 0;
...
UA_StatusCode UA_decodeBinaryMembuf(const UA_ByteString *src, size_t *UA_RESTRICT offset, void *dst, const UA_DataType *dataType) {
	uintptr_t ptr = (uintptr_t) dst;
	UA_StatusCode retval = UA_STATUSCODE_GOOD;
	UA_Byte membersSize = dataType->membersSize;
	for (size_t i = 0; i < membersSize; i++) {
		...
		if (member->isArray) {
			...
			if (retval)
				return retval;
			ptr += sizeof(UA_Int32) + (member->padding & 0x07);
			retval = UA_Array_decodeBinaryMembuf(src, offset, *noElements, (void**) ptr, memberType);
			if (retval != UA_STATUSCODE_GOOD)
                return retval;
			ptr += sizeof(void*);
			continue;
		...
		ptr += member->padding;
		if (!member->namespaceZero) {
			UA_decodeBinaryMembuf(src, offset, (void*) ptr, memberType);
			ptr += memberType->memSize;
			continue;
		}
		...
		case UA_TYPES_GUID:
			...
			break;
		case UA_TYPES_NODEID:
			retval = UA_NodeId_decodeBinaryMembuf(src, offset, (UA_NodeId*) ptr);
			break;
		case UA_TYPES_EXPANDEDNODEID:
			retval = UA_ExpandedNodeId_decodeBinaryMembuf(src, offset, (UA_ExpandedNodeId*) ptr);
			break;
		case UA_TYPES_LOCALIZEDTEXT:
			retval = UA_LocalizedText_decodeBinaryMembuf(src, offset, (UA_LocalizedText*) ptr);
			break;
		case UA_TYPES_EXTENSIONOBJECT:
			retval = UA_ExtensionObject_decodeBinaryMembuf(src, offset, (UA_ExtensionObject*) ptr);
			break;
		case UA_TYPES_DATAVALUE:
			retval = UA_DataValue_decodeBinaryMembuf(src, offset, (UA_DataValue*) ptr);
			break;
		case UA_TYPES_VARIANT:
			retval = UA_Variant_decodeBinaryMembuf(src, offset, (UA_Variant*) ptr);
			break;
		case UA_TYPES_DIAGNOSTICINFO:
			retval = UA_DiagnosticInfo_decodeBinaryMembuf(src, offset, (UA_DiagnosticInfo*) ptr);
			break;
		default:
			// also handles UA_TYPES_QUALIFIEDNAME, UA_TYPES_STRING, UA_TYPES_BYTESTRING,
			// UA_TYPES_XMLELEMENT
			retval = UA_decodeBinaryMembuf(src, offset, (void*) ptr, memberType);
		}
		ptr += memberType->memSize;
	}
	return retval;
